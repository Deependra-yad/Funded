...
search_html = '''              <div class="px-4 py-2 bg-slate-50 border-b border-slate-200">
                  <div class="relative">
                      <i data-lucide="search" class="w-4 h-4 absolute left-3 top-2.5 text-slate-400"></i>
                      <input type="text" id="symbol-search" placeholder="Search strikes (e.g. NIFTY24000CE)..." class="w-full bg-white border border-slate-300 rounded pl-9 pr-3 py-1.5 text-xs font-bold text-slate-700 outline-none focus:border-indigo-500 uppercase">
                  </div>
              </div>'''

new_search_html = '''              <div class="px-4 py-2 bg-slate-50 border-b border-slate-200 relative">
                  <div class="relative">
                      <i data-lucide="search" class="w-4 h-4 absolute left-3 top-2.5 text-slate-400"></i>
                      <input type="text" id="symbol-search" placeholder="Search strikes (e.g. NIFTY24000CE)..." autocomplete="off" class="w-full bg-white border border-slate-300 rounded pl-9 pr-3 py-1.5 text-xs font-bold text-slate-700 outline-none focus:border-indigo-500 uppercase">
                  </div>
                  <div id="search-results" class="absolute left-4 right-4 top-10 bg-white border border-slate-200 rounded-b shadow-lg z-50 hidden max-h-60 overflow-y-auto"></div>
              </div>'''

html = html.replace(search_html, new_search_html)

old_js = '''    document.getElementById('symbol-search').addEventListener('keypress', function(e) {
        if (e.key === 'Enter') {
...
new_js = '''    const searchInput = document.getElementById('symbol-search');
