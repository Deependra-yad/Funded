...
# Use regex to find and replace the HTML part safely
html = re.sub(r'<div class="px-4 py-2 bg-slate-50 border-b border-slate-200">\s*<div class="relative">[\s\S]*?</div>\s*</div>', lambda m: search_div, html)
...
html = re.sub(r'document\.getElementById\(\'symbol-search\'\)\.addEventListener\(\'keypress\', function\(e\) \{[\s\S]*?\}\);', lambda m: js_logic, html)
